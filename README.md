...
`installed_skills()` lists every skill an agent can load, in the project and
in your home folder, whether {ally} installed it or not:
...
It shows what installed each skill, where from, when, when the skill last
changed on GitHub, and whether you have edited it. Skills that were copied in
by hand have no record of where they came from, so those columns are `NA`.

## Your edits are safe

`install_skill()` records a fingerprint of each skill's files. If you edit a
skill, `update_skill()`, `install_skill()` and `link_skills()` stop rather than
overwrite your changes, and tell you which files changed. Pass `force = TRUE`
to overwrite them anyway.

## Agent instructions

Each AI coding assistant reads its own instruction file, where you tell it how
you like to work: `CLAUDE.md` for Claude Code, `AGENTS.md` for Codex and
Posit Assistant, some in your home folder and some in the project.
`list_agent_instructions()` shows which ones you have, and
`edit_agent_instructions()` opens the one you pick in Positron or RStudio:
...
`installed_skills()` looks in the project and your home folder unless you pick
one.
